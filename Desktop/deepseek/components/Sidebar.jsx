...
const Sidebar = ({expand, setExpand}) => {
    ...
    z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
        <div>
            <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
                <Image className={expand ? "w-36" : "w-10"} src={expand ? assets.logo_text : 
                ...
                assets.sidebar_icon} alt='' className='hidden md:block w-7'/>
                 <div className={`absolute w-max ${expand ? "left-1/2 -translate-x-1/2 top-12":
                    "-top-12 left-0"} opacity-0 group-hover:opacity-100 transition
                    ...
                    <div className={`w-3 h-3 absolute bg-black rotate-45 ${expand ?
                        "left-1/2 -top-1.5 -translate-1/2" : "left-4 -bottom-1.5" }`}>                      
                        </div>                   
                 </div>
                </div>
            </div>
  ...
  <div>
    <div className={`flex items-center cursor-pointer group relative ${expand ? 
        ...
        <Image className={expand ? "w-5" : "w-6.5 mx-auto"} src={expand ?
             assets.phone_icon : assets.phone_icon_dull} alt='' />
